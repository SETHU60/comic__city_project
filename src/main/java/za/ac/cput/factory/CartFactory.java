...
public class CartFactory {
    public static Cart buildCart(double totalPrice, List<ComicBook> comicBooks, LocalDate createdDate, LocalDate updatedDate) {
        if (Helper.isValidPrice(totalPrice) || Helper.isComicBooksListNullOrEmpty(comicBooks) || Helper.isNotAfter(createdDate, LocalDate.now()) || Helper.isNotInPast(updatedDate)) {
            return null;
        }
